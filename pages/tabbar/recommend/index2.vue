<template>
	<view class="page">
		<<!-- view class="list-box" v-if="lshow">
			{{list}}
	</view> -->
	<swiper vertical :style="scrollStyle" :current="current" @change="swiperChange" @transition="transition">
		<swiper-item class="h-100" v-for="(item, i) in list" :key="i" :item-id="i.toString()">
			<yingbing-video :autoplay="i == 0" :ref="'video' + i" class="h-100" :poster="item.full_img"
				:src="item.videourl" @play="monitorPlay" @ended="ended" @pause="monitorPause" @timeupdate="timeupdate">

				<template #controls>
					<view class="position-absolute top-half left-half tf-half-xy text-white">
						<u-icon @click="videoPlay(i)" v-if="lshow && item.status == 0" name="play-right-fill"
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 滚动切换
			swiperChange(i) {
				console.log('swiperChange', i)
				if (!("current" in i.detail)) return
